...
def microbial_region_trimming(img_path):
    ...
    return 1
def clean_runs_detect():
    import shutil
    import os
    if os.path.exists('runs/detect/egg_or_not/'):
        shutil.rmtree('runs/detect/egg_or_not/')
...
def is_it_egg():
    ...
    model.load_state_dict(torch.load("..\\finetuning\\runs\\train\egg600\model_weights.pth"))
...
if __name__ == "__main__":
    ...
    # python egg_or_not_gpu_monitor.py --weights runs\train\all\weights\last.pt --save-txt --img 640 --name egg_or_not --exist-ok --conf 0.1 --source
    # python egg_or_not_gpu_monitor.py --weights runs\train\all\weights\last.pt --save-txt --img 640 --name egg_or_not --exist-ok --conf 0.1 --source C:\ex\sen\data\pickup_8times\img\07290845_1_100\07290845_1_100_00.jpg
    ...
    main(opt)
    time_end =time.time()
    yolo_time =time_end - time_sta

    time_sta = time.time()
    microbial_region_trimming(opt.source)
    time_end = time.time()
    microbial_region_trimming_time = time_end - time_sta
    ...
    is_it_egg()
    time_end = time.time()
    efficientnet_time = time_end - time_sta

    all_time_end = time.time()
    all_time = all_time_end - all_time_sta
    ...
    print(f"全体時間 {all_time}")
    print(f"YOLOv5での検出時間 {yolo_time}")
    print(f"物体領域切り出し時間 {microbial_region_trimming_time}")
    print(f"EfficientNet時間 {efficientnet_time}")
